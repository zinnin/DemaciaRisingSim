...
public static class Simulator
{
    ...
    /// Note: multipliers must already be set on the settlement (call ApplyMultipliers first on the full board).
    /// </summary>
    public static ResourceOutput SettlementOutput(Settlement settlement)
    {
        int lumber = 0, stone = 0, metal = 0, petricite = 0;

        foreach (var tile in settlement.Tiles)
        {
            switch (tile)
            {
        ...
        if (settlement.Terrain.HasFlag(TerrainType.Mountain))
        {
        ...
        else if (settlement.Terrain.HasFlag(TerrainType.Border))
        {
            metal += GameConstants.MetalTileValue * GameConstants.BorderMetalBonusTiles;
            lumber = (int)Math.Floor(lumber * settlement.Multiplier);
        }
        else if (settlement.Terrain.HasFlag(TerrainType.Heartland))
        {
            lumber = (int)Math.Floor(lumber * (settlement.Multiplier + GameConstants.HeartlandLumberBonus));
        }
        else
        {
            lumber = (int)Math.Floor(lumber * settlement.Multiplier);
    ...
    /// </summary>
    public static Dictionary<string, Settlement> Permutate(Dictionary<string, Settlement> board)
    {
        ...
        foreach (var settlementName in current.Keys.ToList())
        {
            ...
            var maxTile = settlement.AllowsPetricite ? TileType.Petricite : TileType.Academy;

            for (int tileIndex = 0; tileIndex < settlement.Tiles.Length; tileIndex++)
            {
                var originalTile = settlement.Tiles[tileIndex];

                for (var candidate = TileType.Lumber; candidate <= maxTile; candidate++)
                {
                    settlement.Tiles[tileIndex] = candidate;
                    ...
                    {
                        settlement.Tiles[tileIndex] = originalTile;
    ...
    /// </summary>
    public static Dictionary<string, Settlement> OptimizeBoard(Dictionary<string, Settlement> board)
    {
    ...
    private static bool BoardsEqual(Dictionary<string, Settlement> a, Dictionary<string, Settlement> b)
    {
        ...
        foreach (var kv in a)
        {
            ...
            if (!kv.Value.Tiles.SequenceEqual(bSettlement.Tiles)) return false;
        }
        return true;
    }
}
